...
// empty
// post: boards get the called number marked, and the index of the winning board is
// returned; if there's no winning board, -1 is returned
int calculate_round(int call, int n, Boards& boards) {
...
int main() {
    ...
    int i = 0;
    int winner = -1;
    while (winner == -1 and i < NUMBER_COUNT) {
        winner = calculate_round(numbers[i], BOARD_SIZE, boards);
        ++i;
    }

    int score = board_score(boards[winner], BOARD_SIZE, numbers[i-1]);

    cout << "The winner is #" << winner << " and its score is " << score << endl;
}
